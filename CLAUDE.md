...
- Test runner: vitest (unit + integration); see `vitest.config.ts` and `vitest.integration.config.ts`
- Linting and build: kept clean (`npm run lint` and `npm run build`)
- Error handling: unified with `withErrorHandling` utility pattern
- Source map integration: TS↔JS mapping with LRU TraceMap cache and basename index
- Run `npm run test:integration` to verify full integration suite (long-running)
